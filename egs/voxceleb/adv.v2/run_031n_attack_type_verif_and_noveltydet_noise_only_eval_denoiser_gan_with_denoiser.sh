...
# Eval only - Changed y-dim from 6 to 4
# Eval only - Changed y-dim back to 6 ; threw error
# Eval only - Changed y-dim to 8
# Eval only - Changed y-dim to 5
# Eval only - Changed y-dim to 4
...
list_someknown_dir=data/$sk_attack_type_split_tag"_noise_only"
echo "list_someknown_dir="$list_someknown_dir
# list with all the attacks
list_all_dir=data/$attack_type_split_tag"_noise_only"
echo "list_all_dir="$list_all_dir
args=""
if [ "$resume" == "true" ];then
    args="--resume"
fi
...
echo "Using signature extractor trained using oracle noise"
sign_nnet_reldir=$spknet_name/$sign_nnet_name/$sk_attack_type_split_tag"_noise_only"
sign_nnet_dir=/export/fs02/sjoshi/codes/hyperion/egs/voxceleb/adv.v2/exp/sign_nnets/$sign_nnet_reldir
echo "sign_nnet_dir="$sign_nnet_dir

sign_nnet_reldir_sign_with_denoiser=$spknet_name/$sign_nnet_name/$sk_attack_type_split_tag"_noise_only_with_denoiser"
sign_dir=exp/signatures/$sign_nnet_reldir_sign_with_denoiser
echo "sign_dir="$sign_dir

logits_dir=exp/logits/$sign_nnet_reldir_sign_with_denoiser
echo "logits_dir="$logits_dir

sign_nnet=$sign_nnet_dir/model_ep0020.pth

denoiser_model_path=/export/fs02/sjoshi/codes/hyperion/egs/voxceleb/adv.v2/exp/denoiser_model_someknown/BWE-2/184/14_0.0002_0.0.pt # GAN Model trained by SK leaving out CW attacks
denoiser_model_ctn_layer=8
denoiser_model_load_string=G
denoiser_return_noise=True
denoiser_model_name_predict='B' # Predicts benign
...
echo "Stage 1: Using model trained using oracle noise earlier"
echo "sign_nnet="$sign_nnet
...
#No such file exp/signatures/lresnet34/lresnet34/exp_attack_type_someknown_noise_only_with_denoiser/test/wav.scp


if [ $stage -le 2 ]; then
    echo "Extract attack signatures for known and unknown attacks "
    
    if [ ! -f $list_all_dir/test/wav.scp ]
    then
...
# 	--denoiser_model_path $denoiser_model_path \
# 	--denoiser_model_load_string $denoiser_model_load_string \
# 	--denoiser_model_n_layers $denoiser_model_n_layers \
# 	$sign_nnet $list_someknown_dir/train \
# 	$sign_dir/train
# fi

if [ $stage -le 7 ]; then
    echo "----- Stage 7 - Extracts x-vectors to train plda on known attacks"
    #mkdir -p $list_someknown_dir/train
    #cp $list_someknown_dir/train_wav.scp $list_someknown_dir/train/wav.scp
    #cp $list_someknown_dir/train_utt2attack $list_someknown_dir/train/utt2spk
    nj=20 # Reducing number of jobs because otherwise jobs go to error state in CLSP grid
    steps_xvec/extract_xvectors_from_wav_with_preproc_denoiser.sh \
	--cmd "$xvec_cmd --mem 6G" --nj $nj ${xvec_args} --use-bin-vad false \
	--feat-config $feat_config \
	--denoiser_model_path $denoiser_model_path \
...
be_dir=$sign_dir/train
if [ $stage -le 8 ]; then
    echo "----- Stage 8 - Train PLDA model on known attacks"
    steps_backend/train_be_v1.sh --cmd "$train_cmd" \
        --plda-type splda \
        --y-dim 4 \
	$sign_dir/train/xvector.scp \
...
if [ $stage -le 13 ]; then
    echo "Train PLDA on known attacks without benign samples"
    if [ ! -f $list_someknown_dir/train_nobenign/utt2spk ]
    then
    echo "File does not exist, making it"
    mkdir -p $list_someknown_dir/train_nobenign
    awk '!/benign/' $list_someknown_dir/train/utt2spk > $list_someknown_dir/train_nobenign/utt2spk
