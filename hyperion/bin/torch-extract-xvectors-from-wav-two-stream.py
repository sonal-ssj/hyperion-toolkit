...
def extract_xvectors_two_stream(
    input_spec,
    input_spec_2,
    output_spec,
    vad_spec,
    ...
    **kwargs
    ...
    with DWF.create(output_spec, scp_sep=scp_sep) as writer:
        ...
        with AR(input_spec, **ar_args) as reader:
            ...
            logging.info(
            "opening input stream: {} with args={}".format(input_spec_2, ar_args)
            )
            with AR(input_spec_2, **ar_args) as reader_2:
                ...
                while not reader.eof() and not reader_2.eof():
                    t1 = time.time()
                    key, x0, fs = reader.read(1)
                    key_2, x0_2, fs_2 = reader_2.read(1)

                    # Same sampling freq and utt key ID
                    assert fs == fs_2
                    assert key == key_2
                    ...
                    for aug_id in range(num_augs):
                        ...
                        key_2, x_2 = augment(key0, x0_2, augmenter, aug_df, aug_id)
                        t4 = time.time()
                        ...
                        with torch.no_grad():
                            ...
                            # VAD for first stream and second stream is kept the same at present
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description=(
            "Extracts x-vectors from waveform computing acoustic features on the fly using two-stream data"
        )
    )

    parser.add_argument("--cfg", action=ActionConfigFile)
    parser.add_argument("--input", dest="input_spec", required=True)
    parser.add_argument("--input2", dest="input_spec_2", required=True)
    ...
    extract_xvectors_two_stream(**namespace_to_dict(args))