...
class AudioDataset2(Dataset):
    '''
    this one loads two parallel corpus (for supervised learning)
    '''
    def __init__(
        self,
        audio_path,
        ...
        return_key=False,
        rstrip_key='',
        rstrip_from2=False  # True: extra token is in second dataset so we concat instead of rstrip
    ):
        ...
        self.rstrip_key = rstrip_key
        self.rstrip_from2 = rstrip_from2
        if not self.rstrip_from2:
            for ii in range(len(self.u2c.key)):
                self.u2c.key[ii] = self.u2c.key[ii][:-len(self.rstrip_key)]  # remove string from right
        if rank == 0:
            logging.info("dataset contains %d seqs" % self.num_seqs)
    ...
    def _read_time_durs_file2(self, file_path, file_path2):
        ...
        if not self.rstrip_from2:
            ...
            self._seq_lengths = np.minimum(nf_df.loc[self.u2c.key, 1].values, nf_df2.loc[self.u2c.key+self.rstrip_key, 1].values)
    ...
    def _get_random_chunk(self, index):
        ...
        # first dataset
        if not self.rstrip_from2:
            x, fs = self.r.read([key+self.rstrip_key], time_offset=time_offset, time_durs=read_chunk_length)
        else:
            x, fs = self.r.read([key], time_offset=time_offset, time_durs=read_chunk_length)
        ...
        if not self.rstrip_from2:
            ...
            x2, fs2 = self.r2.read([key+self.rstrip_key], time_offset=time_offset, time_durs=read_chunk_length)
        x2 = x2[0]
        fs2 = fs2[0]
