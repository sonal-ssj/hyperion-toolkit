...
if [ $# != 3 ] && [ $# != 4 ]; then
  echo "Usage: $0 [options] <nnet-model> <data> <data2> <xvector-dir> [<data-out-dir>]"
...
nnet_file=$1
data_dir=$2
data_dir2=$3 #Added for oracle perturbation
output_dir=$4
data_out_dir=$5
...
if [ $stage -le 3 ]; then
    ...
    if [ ! -f $data_out_dir/utt2num_frames ]
        then
        cp $output_dir/utt2num_frames $data_out_dir
        else
        echo "${data_out_dir}/utt2num_frames exists, not copying"
    fi
    else
