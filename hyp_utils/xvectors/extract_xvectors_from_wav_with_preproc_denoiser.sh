...
if [ $# != 3 ] && [ $# != 4 ]; then
  ...
  echo "  --denoiser_model_path                            # Path to denoiser model"
  echo "  --denoiser_model_load_string                     # Denoiser model load string, G, model_state_dict"
  echo "  --denoiser_model_n_layers                        # Denoiser model number of layers" 
  echo "  --denoiser_model_n_layers                        # Denoiser model number of layers" 
  echo "  --denoiser_name_predict                          # What does the trained denoiser predict? choices={'B','P','BP'} for {'benign','adv perturb','benign,adv perturb'}"
fi
...
if [ -n "$denoiser_model_load_string" ];then
    args="${args} --denoiser_model_n_layers $denoiser_model_n_layers"
fi
...
if [ $stage -le 0 ];then
	...
	torch-extract-xvectors-from-wav-with_preproc_denoiser.py \
    ...
    --denoiser_name_predict $denoiser_name_predict \
    set -e
fi
